...
def parse_args():
    ...
    parser.add_argument('--dataset_dir', required=True, type=str, help='Path to .npy format dataset')
...
def room2blocks_plus_normalized(data_label, num_point, block_size, stride,
    ...
    assert np.max(data_label[:, -2]) < 256
    assert np.max(data_label[:, -1]) < 256
    ...
    for i, file in enumerate(files):
        try:
            ...
            if h5_filename.exists():
                print('{} exists, skip'.format(str(h5_filename)))
                continue
